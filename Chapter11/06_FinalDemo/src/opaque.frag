...
void main() {
  ...
  // Bistro has zero-length vertex normals: normalize() would turn them into NaNs, and ray queries
  // reject non-finite origins (VUID-RuntimeSpirv-OpRayQueryInitializeKHR-06351)
  const float normalLen2   = dot(normal, normal);
  const bool  isNormalValid = normalLen2 > 0.0 && normalLen2 < 1.0e38; // false for 0, Inf and NaN
  vec3 nGeom = isNormalValid ? normal * inversesqrt(normalLen2) : vec3(0.0, 1.0, 0.0);

  // ray-traced ambient occlusion with spatial hashing (replaces screen-space SSAO)
  float occlusion = (pc.light.enableAO != 0u && isNormalValid) ? computeAO(worldPos, nGeom, gl_FragCoord) : 1.0;
