...
{
  const int level = scene.hierarchy_[node].level_;
...
bool recalculateGlobalTransforms(Scene& scene)
{
  ...
  for (int i = 1; i < MAX_NODE_LEVEL; i++) {
    ...
    wasUpdated |= !scene.changedAtThisFrame_[i].empty();
...
void shiftNodes(Scene& scene, int startOffset, int nodeCount, int shiftAmount)
{
  ...
  //	std::transform(scene.hierarchy_.begin() + startOffset, scene.hierarchy_.begin() + nodeCount, scene.hierarchy_.begin() + startOffset,
  //shiftNode);
    ...
    bool mergeMeshes, bool mergeMaterials)
{
  ...
  for (const Scene* s : scenes) {
    ...
    const int nodeCount = (int)s->hierarchy_.size();
