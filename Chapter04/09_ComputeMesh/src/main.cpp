...
void renderGUI(lvk::TextureHandle texture)
{
  ...
  ImGui::Begin("Torus Knot params", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse);
  {
    ...
    for (size_t i = 0; i != g_MorphQueue.size(); i++) {
      ...
      ImGui::Text("  P = %u, Q = %u %s", g_MorphQueue[i].first, g_MorphQueue[i].second, isLastElement ? "<---" : "");
    }
  }
  ImGui::End();
...
int main()
{
...

  {
    ...
    lvk::Holder<lvk::ShaderModuleHandle> compMesh    = loadShaderModule(ctx, "Chapter04/09_ComputeMesh/src/main_mesh.comp");
    lvk::Holder<lvk::ShaderModuleHandle> compTexture = loadShaderModule(ctx, "Chapter04/09_ComputeMesh/src/main_texture.comp");
    ...
    lvk::Holder<lvk::ShaderModuleHandle> vert = loadShaderModule(ctx, "Chapter04/09_ComputeMesh/src/main.vert");
    lvk::Holder<lvk::ShaderModuleHandle> geom = loadShaderModule(ctx, "Chapter04/09_ComputeMesh/src/main.geom");
    lvk::Holder<lvk::ShaderModuleHandle> frag = loadShaderModule(ctx, "Chapter04/09_ComputeMesh/src/main.frag");
